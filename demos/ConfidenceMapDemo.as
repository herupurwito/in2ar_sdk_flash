package
{
    ...
    import ru.inspirit.asfeat.IConfidenceMap;

	/**
	 * @author Eugene Zatepyakin
	 */
	[SWF(width='640',height='600',frameRate='25',backgroundColor='0xFFFFFF')]
	public final class ConfidenceMapDemo extends Sprite
	{
		...
		public var confMapScale:Number;
		public var confMap:IConfidenceMap;
		...
		public function ConfidenceMapDemo()
		{
		...
		protected function initASFEAT():void
		{
			...
			// get Confidence map class instance
			confMap = asfeatLib.getConfidenceMapInstance();
			// we downscale reference image because 
			// in real life we will never detect it
			// at original size on the screen
			confMapScale = 1.0 / 2.5;
			var confMapWidth:int = refImg.width * confMapScale;
			var confMapHeight:int = refImg.height * confMapScale;
			...
			// init Confidence Map
			confMap.setup(confMapWidth, confMapHeight);
			confMap.setReference(refImg);
		...
		protected function onModelDetected(e:ASFEATDetectionEvent):void
		{
			...
			for(var i:int = 0; i < n; ++i)
			{
                ...
                confMap.updateMap(_cambuff, ref.TLx, ref.TLy, ref.TRx, ref.TRy, ref.BRx, ref.BRy, ref.BLx, ref.BLy);
			...
			// lets check word "TEST" in marker image
			var recConf:Number = confMap.getConfidence(new Rectangle(164 * confMapScale, 132 * confMapScale,
																	209 * confMapScale, 76 * confMapScale));
			_txt.appendText('\nword "TEST" confidence: ' + recConf.toFixed(3));
